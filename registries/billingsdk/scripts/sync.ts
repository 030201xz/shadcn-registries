/**
 * Sync Script for BillingSDK
 *
 * From `https://billingsdk.com/r` Get all components and save them locally
 * Responsibility: Purely download, keep the source data as is
 *
 * Usage:
 *   bun run registries/billingsdk/scripts/sync.ts
 *   bun run registries/billingsdk/scripts/sync.ts --only=pricing-table-one
 *   bun run registries/billingsdk/scripts/sync.ts --dry-run
 */
