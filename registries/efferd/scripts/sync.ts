/**
 * Sync Script for Efferd
 *
 * From `https://efferd.com/r` Get all components and save them locally
 * Responsibility: Purely download, keep the source data as is
 *
 * Note: Some items in the registry index don't have individual JSON endpoints.
 * These will be skipped during sync.
 *
 * Usage:
 *   bun run registries/efferd/scripts/sync.ts
 *   bun run registries/efferd/scripts/sync.ts --only=auth-1,contact-1
 *   bun run registries/efferd/scripts/sync.ts --dry-run
 */
